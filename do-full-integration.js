...
// ===== Step 1: Add the require for coinflip-dashboard-static =====
...
// ===== Step 2: Replace PAGE_HTML =====
const newPageHtml = fs.readFileSync('coinflip-dashboard-pagehtml.txt', 'utf8');

// Find the PAGE_HTML (the one after "// ── Web GUI: dashboard page")
...
while (true) {
  ...
  if (idx === -1) break;
  // Check if it's the PAGE_HTML (not LOGIN_HTML) by looking at context
...
result = before + newPageHtml + after;

// ===== Step 3: Add the API routes and static file routes =====
// Find the place after the /play route handler (before res.writeHead(404); res.end('not found'))
...
// ===== Step 4: Add COINFLIP button to sidebar (it's already in the new PAGE_HTML but let's verify) =====
// The new PAGE_HTML already includes the coinflipbtn
