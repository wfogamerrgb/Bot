...
// Extract just the template literal from the new file
...
// Find the end by looking for the comment that follows PAGE_HTML in the original file
// The original has: `...`\n\n// ── Web GUI server (native http + ws, session-cookie auth)
const commentMarker = '// ── Web GUI server (native http + ws, session-cookie auth)';
let searchPos = startIdx + startMarker.length;
const commentIdx = result.indexOf(commentMarker, searchPos);
if (commentIdx === -1) {
  console.error('Could not find comment marker after PAGE_HTML');
  process.exit(1);
}

// Now find the backtick right before this comment (with optional whitespace)
let endIdx = commentIdx - 1;
while (endIdx >= searchPos && /\s/.test(result[endIdx])) {
  endIdx--;
}
if (result[endIdx] !== '`') {
  console.error('Expected backtick before comment, found:', result[endIdx]);
  process.exit(1);
}
console.log('Found end backtick at:', endIdx, 'comment at:', commentIdx);
